"""MIDI sequence processing and event representation."""
...
USE_VELOCITY = True
USE_TEMPO = True  # Enable tempo events to match Magenta's original implementation
BEAT_LENGTH = 60 / DEFAULT_TEMPO
DEFAULT_TIME_SHIFT_BINS = 1.15 ** np.arange(100) / 65  # 100 bins to match Magenta (originally 32)
DEFAULT_VELOCITY_STEPS = 32
DEFAULT_TEMPO_BINS = np.linspace(30, 250, 80)  # 80 tempo bins (30-250 BPM) to match Magenta
...
class EventSeq:
    ...
    def feat_dims():
        ...
        if USE_TEMPO:
            feat_dims['set_tempo'] = len(EventSeq.tempo_bins)
        return feat_dims
    ...
    def to_note_seq(self):
        ...
        velocity_bins = EventSeq.get_velocity_bins()
        tempo = DEFAULT_TEMPO  # Track current tempo (though not directly applied to notes)
        if USE_TEMPO:
            tempo_bins = EventSeq.get_tempo_bins()
        ...
        for event in self.events:
            ...
            elif event.type == 'set_tempo' and USE_TEMPO:
                index = min(event.value, len(tempo_bins) - 1)
                tempo = tempo_bins[index]
